...
public class ClienteService implements service.IService<Cliente> {

    @Override
    public Cliente save(Cliente cliente) throws SQLException, ClassNotFoundException {
        Connection conn = ConnectionManager.getConnection();
        PreparedStatement insert = conn.prepareStatement("insert into folha_owner.clientes values(?,?,?,?,?,?,?)");
    ...
    public Cliente update(Cliente cliente) throws SQLException, ClassNotFoundException {
        Connection conn = ConnectionManager.getConnection();
        PreparedStatement update = conn.prepareStatement("UPDATE folha_owner.clientes SET nome = ?, documento = ?, altura = ?,peso = ? WHERE id = ?");
    ...
    public Cliente delete(Cliente cliente) throws SQLException, ClassNotFoundException {
        Connection conn = ConnectionManager.getConnection();
        PreparedStatement statement2 = conn.prepareStatement("DELETE FROM folha_owner.clientes WHERE ID = ?");
